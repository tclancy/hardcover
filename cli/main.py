...
from cli.db import get_connection, init_db, upsert_book, upsert_in_dover, get_all_books, record_decision
...
def _ask_user(results: list[dict], book: dict, query: str, conn) -> None:
    ...
    choice = typer.prompt("Enter result number to confirm match, or 's' to skip").strip().lower()

...
def test_no_match_scores_low():
    result = score_match(
        hc_title="Dune",
        hc_author="Frank Herbert",
        koha_title="Foundation",
        koha_author="Asimov, Isaac",
        koha_year=None,
    )
    assert result < 0.3
...
def test_close_title_scores_medium():
    result = score_match(
        hc_title="The Night Circus",
        hc_author="Erin Morgenstern",
        koha_title="Night Circus",
        koha_author="Morgenstern, Erin",
        koha_year=None,
    )
    assert result > 0.7
